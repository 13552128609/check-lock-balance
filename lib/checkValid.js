...
async function checkValid({ groupId, smgAddr, gpkAddr, rpcUrl, verbose, deps }) {
  ...
  async function mapLimit(limit, arr, fn) {
  ...
  logger.log("[checkValid] fetching gpkCount/groupMap.smNumber/threshold...");
  const [gpkCountRaw, groupInfo, thresholdRaw] = await Promise.all([
    gpkSc.getGpkCount(groupId),
    gpkSc.groupMap(groupId),
    smgSc.getThresholdByGrpId(groupId),
  ]);

  const gpkCount = Number(gpkCountRaw);
  ...
  const gpkTransformer = deps?.gpkTransformer;
  const gpkRaw = await Promise.all(makeRange(fetchCount).map((i) => gpkSc.getGpkbyIndex(groupId, i)));
  const gpk = typeof gpkTransformer === "function"
    ? gpkRaw.map((g, i) => gpkTransformer(g, { groupId, index: i }))
    : gpkRaw;
  ...
  const CONCURRENCY_SHARES = 5;
  const smIndices = makeRange(smNumber);

  const perSmShares = await mapLimit(CONCURRENCY_SHARES, smIndices, async (smIndex) => {
    if (smIndex === 0 || (smIndex + 1) % 10 === 0 || smIndex === smNumber - 1) {
      logger.log(`[checkValid] gpkShare progress: smIndex=${smIndex + 1}/${smNumber}`);
    }
    const shares = await Promise.all(
      makeRange(fetchCount).map((gpkIndex) => gpkSc.getGpkSharebyIndex(groupId, smIndex, gpkIndex))
    );

    for (let gpkIndex = 0; gpkIndex < shares.length; gpkIndex++) {
      const len = hexToBuf(shares[gpkIndex]).length;
      if (len !== 64) {
        throw new Error(`expected 64 bytes, got ${len}`);
      }
    }
  ...
  for (let smIndex = 0; smIndex < smNumber; smIndex++) {
    for (let gpkIndex = 0; gpkIndex < fetchCount; gpkIndex++) {
      gpkShares[gpkIndex][smIndex] = perSmShares[smIndex][gpkIndex];
    }
  }
  ...
  const CONCURRENCY_PKS = 10;
  const wkingPks = await mapLimit(CONCURRENCY_PKS, makeRange(smNumber), async (smIndex) => {
    if (smIndex === 0 || (smIndex + 1) % 10 === 0 || smIndex === smNumber - 1) {
